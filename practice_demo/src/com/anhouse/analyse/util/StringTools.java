...
//		stepId = Pattern.compile("[^0-9]").matcher(stepId).replaceAll("");
		if(stepId == null || "".equals(stepId.trim())) return 0;
	...
	/** 忽视大小写，替换search字符  */
	public static String replaceIgnoreCase(String src, String search, String rep) {
		if(rep == null) rep = "";
		if(src == null || search == null) return src;
		if( StringUtils.containsIgnoreCase(src, search) ) {
			int start = StringUtils.indexOfIgnoreCase(src, search);// >= 0
			String temp = StringUtils.substring(src, start, (start+search.length()));
...
//////////////////////////////////////////////////////////////////////////////////////
	...
	/** 获取从searchStr开始往后的子字符串   */
	public static String subBetweenString(String str, String start, String end) {
		if(str == null) return null;
	...
	public static Set<String> toSet(String src, String split){
		...
		if( !isTrimNull(src, split) ){
			if(src.contains(split)){
				String[] fields = StringUtils.split(src, split); //src.split( split );
	...
	/** 比较两个<基本类型对象>是否一样 */
	public static boolean isSame(Object base, Object real){
		if(base == null && real == null){
			return true;
		}else if(base != null && real != null){
			if("".equals(base) && "".equals(real)){
				return true;
			}else if(base instanceof String && real instanceof String){
				return isSame(base.toString(), real.toString(), false);
			}else if(base instanceof Number && real instanceof Number){
				if(base instanceof Integer && real instanceof Integer){
					int b = Integer.valueOf(base.toString().trim()).intValue();
					int r = Integer.valueOf(real.toString().trim()).intValue();
					return b == r;
				}else if(base instanceof Long && real instanceof Long){
					long b = Long.valueOf(base.toString().trim()).longValue();
					long r = Long.valueOf(real.toString().trim()).longValue();
					return b == r;
				}else if(base instanceof Float && real instanceof Float){
					float b = Float.valueOf(base.toString().trim()).floatValue();
					float r = Float.valueOf(real.toString().trim()).floatValue();
					return b == r;
				}else {//if(base instanceof Double && real instanceof Double){
					double b = Double.valueOf(base.toString().trim()).doubleValue();
					double r = Double.valueOf(real.toString().trim()).doubleValue();
					return b == r;
				}
			}else {//其他定义类对象
				return false;
			...
			if(base == null && "".equals(real)){
				return true;
			}else if(real == null && "".equals(base)){
				return true;
			}
		}
		return false;
	 ...
	 * @param isNum : 是否是数值比对  */
	public static boolean isSame(String thisFld, String otherFld, boolean isNum){
		if("".equals(thisFld))
			thisFld = null;
		if("".equals(otherFld))
			otherFld = null;
		
		if(thisFld == null && otherFld == null){
			return true;
			...
			thisFld = StringUtils.trim(thisFld);
			otherFld = StringUtils.trim(otherFld);
			if( isNum ){
				double t = Double.valueOf(thisFld).doubleValue();
				double o = Double.valueOf(otherFld).doubleValue();
				return (t == o);
			}else{
				return StringUtils.equals( thisFld, otherFld);
			}
		}
		return false;
	}
	...
	/** 比对两个Byte[]数组的值是否相等  */
	public static boolean isSame(byte[] thisFld, byte[] otherFld){
		boolean flag = true;
		...
		if(thisFld == null && otherFld != null){
			...
			if(thisFld.length != otherFld.length){
				flag = false;
			}else{
				for(int i=0; i < thisFld.length; i++){
					if(thisFld[i] != otherFld[i]){
						return false;
					}
				}
			}
	...
	public static String toCharset(String str, String srcCharset, String toCharset) {
		if(str != null) {
			try {
				if( !StringTools.isTrimNull(str) ) {
					if( !StringTools.isTrimNull(srcCharset) )
						...
						return new String(str.getBytes("ISO-8859-1"), toCharset);
				}
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
	...
	/**字符串反序列化
	 * @param charset : UTF8  */
	public static String deserialize(byte[] bytes, String charset) {
		if(bytes == null) {
			return null;
		} else {
			if( StringUtils.isBlank(charset) ) charset = "UTF8";
	...
	/** 字符串序列化
	 * @param charset : UTF8  */
	public static byte[] serialize(String string, String charset) {
     ...
     * 去除首尾空格和其中的特殊字符
     * @param str
     * @param specialChar指定去除其中的特殊字符
     * @return */
    public static String trimSpecialChar(String str, String specialChar) {
        return StringUtils.remove(StringUtils.trim(str), specialChar);
    }
    
    
    /**
	 * @param 取多个字符串中第一个不为空的值，若都为空，则返回null
	 * @return */
